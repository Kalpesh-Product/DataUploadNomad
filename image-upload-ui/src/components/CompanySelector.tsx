...
export default function CompanySelector({ companies }: Props) {
  const countries = Array.from(new Set(companies.map((c) => c.country)));
  const companyTypes = Array.from(new Set(companies.map((c) => c.companyType)));
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (payload: { companyId: string; images: File[] }) => {
      const formData = new FormData();
      formData.append("companyId", payload.companyId);
      payload.images.forEach((img) => formData.append("images", img));

      const res = await axiosInstance.post(
        "/company/bulk-add-company-images",
        formData
      );
      reset();
      return res.data;
        ...
        >
          {() => (
            <div className="relative mt-1">
              <Listbox.Button
                className={classNames(
                  !selectedType ? "bg-gray-100 cursor-not-allowed" : "bg-white",
